package Oa::Buying::Controller::Goods;
use Moose;
...
Oa::Buying::Controller::Goods - Catalyst Controller
...
=x
	$c->stash->{dropdown} = {
		way      => $db->master('way'),
		billing  => $db->master('billing'),
	};
=cut
    $c->stash->{history} = ['75','76','77'];
    $c->stash->{success} = $message;
    $c->stash->{template} = 'goods-edit.tt';
